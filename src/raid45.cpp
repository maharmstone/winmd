...
static NTSTATUS __stdcall io_completion_raid45(PDEVICE_OBJECT, PIRP Irp, PVOID ctx) {
    io_context_raid45* context = (io_context_raid45*)ctx;
...
static void paranoid_raid5_check(set_pdo* pdo, uint64_t parity_offset, uint32_t parity_length) {
    ...
    for (uint32_t i = 0; i < pdo->array_info.raid_disks; i++) {
        io_context_raid45* last = (io_context_raid45*)ExAllocatePoolWithTag(NonPagedPool, sizeof(io_context_raid45), ALLOC_TAG);
        if (!last) {
            ERR("out of memory\n");
            goto end;
        }
