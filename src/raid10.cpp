...
NTSTATUS set_pdo::read_raid10(PIRP Irp, bool* no_complete) {
    ...
    if (is_offset) {
        Status = read_raid10_offset(this, Irp, no_complete);
        goto end2;
    }

    if (array_info.raid_disks % near != 0) {
        Status = read_raid10_odd(this, Irp, no_complete);
        goto end2;
    }
    ...
    if (skip_first != 0) {
        ...
        for (uint32_t j = 0; j < far; j++) {
            for (uint32_t i = 0; i < near; i++) {
                ...
                new (last) io_context(child_list[((startoffstripe * near) + i + (j * near)) % array_info.raid_disks], 0, 0);
